...
- `content.js` — content script; on `initializePopup` returns `scrape.extractClinVarData(document)`, AND on page load draws in-page controls on each SCV row (see in-page features below). When a page-load scrape comes back empty (no `variation_id`), it logs a `dom-seams.js` health report (`reportSeamDrift`) so the drifted DOM seam self-reports in the console.
